...
```text
GET /api/research/news?limit=80&symbols=600519,000001,300750
```

- 新闻池聚合财新数据通市场新闻、最近可用日期的央视新闻联播宏观新闻，以及最多 8 只 A 股的东方财富个股新闻；默认观察池不再使用 `stock_news_em()` 的单只股票默认值。
- 单次最多返回 80 条，按链接或规范标题去重，并返回 `macro / market / company` 分类、来源覆盖、请求标的、原始数量和去重数量。
- 任一来源失败时保留其他真实来源并返回部分降级警告；全部来源失败时返回空数组，不使用静态或模拟标题补位。
- 非空结果按标的列表和 limit 在 Python 进程内缓存 15 分钟；只返回最多 240 字摘要，不把新闻全文写入磁盘。
- 关键词情绪只用于界面标签，不是上涨概率，也不读取账户或生成订单。
...
- 快照观察池固定为 16 个国内主连代码，历史端点允许一次请求完整观察池；客户端不能请求任意未知代码或超过观察池上限。
...
#### 获取全球指数、A 股指数历史与 BTC/ETH 快照

```text
GET /api/market/global?limit=10
GET /api/market/global/history?symbols=DJI,SPX,IXIC,HSI,N225,KOSPI&days=500
GET /api/market/index-history?symbols=SH000300&days=500
GET /api/market/crypto/quotes
```

- 全球指数由服务端白名单控制，覆盖道琼斯、标普 500、纳斯达克、恒生、日经 225、KOSPI 和主要欧洲指数；客户端不能透传任意供应商代码。
- 全球历史优先东方财富，按地区回退到新浪美国指数或全球指数日线；每条快照保留市场会话日期、时区、报价类型、实际来源和抓取时间。
- A 股指数历史只允许 `SH000001`、`SZ399001`、`SZ399006` 和 `SH000300`，用于时间安全的研究对齐，不接受个股代码。
- 数字资产只归一化 AkShare 当前返回的 BTC/USD 与 ETH/USD 24 小时快照。当前没有同口径历史端点，源失败时返回空数组和警告，不使用期货或静态价格替代。
- 以上端点均只读，不读取外盘或数字资产账户、不生成订单，也不会提高 A 股正式 paper 仓位。
...
- 全球指数快照兼容 AkShare 当前 `index_global_spot_em()` 与旧版函数名；全球历史按地区使用东方财富和新浪结构化接口回退。
- BTC/ETH 快照使用 `crypto_js_spot()`，仅保留受控币种且不虚构缺失历史。
...
- 后续请求从内存缓存读取，延迟 < 10ms
- 缓存按 TTL 自动刷新，默认 3 秒
- 板块、股票/港股/期货/指数历史日线、非空多源新闻、全球/数字资产/期货快照和非空新股表默认缓存 15 分钟，避免页面刷新重复抓取公开站点
