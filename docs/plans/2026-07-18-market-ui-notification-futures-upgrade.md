...
- [x] 为每个主连序列测试时间安全的 5 日条件统计：结构、相似样本数、上涨/下跌/震荡频率、中位收益、最大有利/不利幅度和失效条件。
- [x] 把请求扩大到受控上限 16 个品种和 500 日；样本少于 20 时返回 `insufficient`，不得虚构频率。
- [x] 将页签升级为“期货研判”，先展示可用研判摘要，再展示完整行情表；空单元格使用 `—`，缺失原因集中显示。
- [x] 明示主连连续序列、历史条件频率、只读研究、不读取期货账户和不生成订单。
- [x] 运行服务端研究测试（7 项通过）和前端构建（2,303 模块转换通过）。
...
- [x] 将真实前复权验证池从最多 8 只扩为 12 只、历史保持 500 日。
- [x] 在现有 10 个固定参数策略之外增加 MACD、A 股强势回踩、网格和定投四种独立逻辑，共 14 种；仍在三个不重叠窗口验证。
- [x] 更新 Fastify 查询边界、前端请求和契约测试，并在 UI 显示策略数、标的数和总验证窗口数。
- [x] 策略与 Fastify 测试 46 项、浏览器 API 客户端测试 10 项通过。
...
- [x] 查询只对网络错误和 5xx 做两次短退避重试，4xx 和认证失败不重试。
- [ ] 刷新失败但已有数据时保留上次成功结果，显示“缓存数据 + 更新时间”，不让整个模块消失。
  - 2026-07-18 最小切片：港股观察已完成；后台刷新失败时保留上次成功报告，并显示缓存状态与最后成功更新时间。其他研究模块留待后续独立处理。
  - 2026-07-18 最小切片：期货研判已复用同一刷新展示契约，刷新失败时继续展示缓存报告、缓存来源状态与最后成功更新时间；其余研究模块留待后续独立处理。
  - 2026-07-18 最小切片：A 股策略多窗口稳健性面板已复用刷新展示契约；后台刷新失败时保留真实历史报告，并显示缓存状态与最后成功更新时间。其他研究模块留待后续独立处理。
  - 2026-07-18 最小切片：A 股五日变盘雷达已复用刷新展示契约；后台刷新失败时保留候选表格，并显示最后成功更新时间。其他研究模块留待后续独立处理。
  - 2026-07-18 最小切片：板块展望与形态识别已迁移到统一 `ResearchQueryState`；后台刷新失败时保留板块和个股研究结果，并显示最后成功更新时间。其他研究模块留待后续独立处理。
  - 2026-07-18 最小切片：新股申购研究已复用同一刷新展示契约；后台刷新失败时保留新股列表，并显示最后成功更新时间。其他研究模块留待后续独立处理。
  - 2026-07-18 最小切片：按名称或代码的个股趋势研判已复用同一刷新展示契约；后台刷新失败时保留图表、周期研判和最后成功更新时间，首次查询失败仍显示阻断错误。其他研究模块留待后续独立处理。
  - 2026-07-18 最小切片：真实新闻与外围市场面板已复用同一刷新展示契约；后台刷新失败时保留新闻、外围影响和最后成功更新时间，首次加载失败显示阻断错误。其他研究模块留待后续独立处理。
  - 2026-07-18 最小切片：新股申购研究已迁移到统一 `ResearchQueryState`；首次加载、无缓存失败和缓存刷新失败不再维护重复 JSX，现有真实发行列表继续保留。
