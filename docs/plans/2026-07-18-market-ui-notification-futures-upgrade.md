...
- [ ] 测试 `contentType=2`，并验证标题、今日结论、精确动作、动作依据、账户、有效市场证据、数据质量和边界声明分区。
- [ ] 导出纯 formatter，统一 HTML 转义；只把真实有效板块放入正文，空值、重复警告和“暂不可用”集中到数据质量区。
- [ ] 动作区逐条包含标的、数量、参考价、预计金额和计划原因；无动作时明确给出继续观察原因。
- [ ] 运行通知相关 Vitest，预期通过且 SPT 不出现在错误或审计内容中。
