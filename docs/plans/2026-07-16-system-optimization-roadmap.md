...
- [x] 将桥接缓存从“完整参数字符串”升级为按市场、代码、复权和交易日窗口存储的单序列缓存，使不同批次能复用同一标的历史。
- [x] 为相同在途请求建立 single-flight 合并；同一代码冷启动期间只允许一个上游请求。
- [x] 缓存保留 `freshUntil` 和 `staleUntil`；fresh 直接返回，stale 立即返回旧结果并后台刷新，超出 stale 才阻塞等待。
- [ ] 港股页面先返回真实快照和 `historyStatus=loading`，完整历史报告通过独立查询更新，避免 10 只历史阻塞整个模块。
- [x] 缓存键必须包含来源、复权、截止交易日和窗口，不允许把旧交易日结果伪装成当前数据。
- [x] 增加缓存命中、single-flight、后台刷新、上游失败保留旧结果和过期淘汰测试。
...
- `D:\conda\python.exe -m pytest akshare-bridge\test_research_cache.py -q`：7 passed，1 个 `.pytest_cache` 写入权限警告（包含过期淘汰覆盖）。
- `D:\conda\python.exe -m pytest akshare-bridge\test_bridge.py akshare-bridge\test_research_cache.py -q`：83 passed，1 个既有 Starlette TestClient/httpx 弃用警告，1 个 `.pytest_cache` 写入权限警告。
- `git diff --check`：通过，仅输出工作树 LF/CRLF 转换警告。
