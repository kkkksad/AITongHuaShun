...
- [ ] 尚未接入 `akshare-bridge/main.py`；过期淘汰、港股历史独立查询和耗时指标留给后续 Task 2 切片。

验证结果：

- `D:\conda\python.exe -m pytest akshare-bridge\test_research_cache.py -q`：6 passed，1 个 `.pytest_cache` 写入权限警告。
- `D:\conda\python.exe -m pytest akshare-bridge\test_bridge.py akshare-bridge\test_research_cache.py -q`：82 passed，1 个既有 Starlette TestClient/httpx 弃用警告，1 个 `.pytest_cache` 写入权限警告。
