...
- [x] **Step 3: 提交并部署**：提交 `533c57d` 已推送，并使用受限归档入口部署；三容器健康。
- [x] **Step 4: 生产验收**：运行时确认目标 2、上限 4、单轮 1、paper 模式和真实交易关闭；桥接队列空闲，持久化账户未重置且没有伪造成交。匿名业务接口继续返回 401；因 IP 自签名证书阻止浏览器复用登录态，本轮未读取登录后的状态 JSON，待下一交易时段通过页面目标状态继续观察。
