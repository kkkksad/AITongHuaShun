...
Fastify 侧还会在 `server/research/bridgeRequest.ts` 对只读桥接请求做最多 64 项的内存 Promise 缓存。相同 URL 和凭据作用域的并发读取会合并，失败请求立即移除；健康结果的 TTL 从请求完成时开始计算，当前行情快照使用 1 至 2 分钟，历史日线使用 5 至 10 分钟，新股申购元数据使用 30 分钟。HTTP 200 中带非空 `warning` 的部分/降级结果最多缓存 5 秒（调用方设置更短 TTL 时保留更短值），避免预热中的空历史被长期复用；这表示下一次请求可以重试，不改变自动执行器本身的轮询周期。未完成的请求不按结果 TTL 过期，也不被 LRU 淘汰；64 个缓存槽都在执行时，新 key 直接读取而不占缓存槽。该层不写磁盘，也不缓存账户、持仓、订单、审计或任何修改请求。新闻面板使用 `scope=news&newsLimit=40&symbolLimit=3`，健康结果 10 分钟内复用并每 15 分钟后台刷新，新闻刷新不再重复读取全球市场。
