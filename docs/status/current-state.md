...
- 2026-08-22 登录跳回修复：线上历史配置的 `WEB_ORIGIN` 仍是服务器 IP，导致从 `kairosq.cn` 登录后 WebSocket 来源校验失败并被前端误判为会话失效。新增可信反代来源判断，接受配置来源或受信任反代转发的实际 HTTPS Host；WebSocket 来源拒绝与会话失效使用不同关闭原因，前端不再因来源错误闪回登录页。
