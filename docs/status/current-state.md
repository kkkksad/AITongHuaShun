...
- **Paper 关键历史优先级** —— 当前计划先完成有界的 A 股核心研究，再启动新闻和全球市场辅助研究；核心池限制为 6 个分散行业和普通模式最多 8 只优先股票，活动目标开启时最多 12 只，持仓优先，其后交错纳入策略候选与质量候选。此改动减少 AkShare 历史队列冷启动竞争，不缓存包含账户状态的完整计划，也不把降级数据提升为可交易证据。
- **Paper 交易漏斗与普通覆盖升级（2026-09-21）** —— Paper 计划和自动执行快照新增候选、可买、历史覆盖、策略命中、计划、提交、成交漏斗；有限阻塞码用于区分行情、历史、策略、可负担性、费用、现金、仓位、阶段、风控、重复和执行问题，原始规则文本继续保留。普通模式核心历史池提高为 8 只，活动目标开启时仍最多 12 只；此改动不放宽 T+1、费用、现金、仓位、熔断、阶段预算、每日上限或真实交易关闭边界。
...
- JSON 交易历史默认只保留最近 7 天；生产环境模板已调整为有界 30 天，以覆盖上一完整交易周复盘。过期清理仍会缩短订单幂等查询和审计回看窗口，需要长期研究的汇总结果应另行导出，不应依赖无限增长的运行状态文件。
...
- 2026-08-22 登录跳回修复：线上历史配置的 `WEB_ORIGIN` 仍是服务器 IP，导致从 `kairosq.cn` 登录后 WebSocket 来源校验失败并被前端误判为会话失效。新增可信反代来源判断，接受配置来源或受信任反代转发的实际 HTTPS Host；WebSocket 来源拒绝与会话失效使用不同关闭原因，前端不再因来源错误闪回登录页。提交 `54af637` 已通过 GitHub Actions `32550879081` 部署，主页和健康接口已恢复 200。
- 2026-08-23 研究可靠性与加载速度升级：Paper 计划把新闻、全球市场和加密等可选辅助研究与核心研究并行启动，并设置 1.5-4 秒有界等待；超时或失败返回明确的 `degraded/neutral` 研究报告，不伪造行情、不改变新增仓位权限。研究管线、每日任务中心和研究页签请求错峰，Paper 研究结果按 TradingSystem 做 30 秒 single-flight/TTL 缓存，失败缓存会淘汰。
- 2026-08-23 缓存失效修复：研究缓存键不再使用每秒变化的行情 `sequence`，而只使用研究配置和有界优先研究池；账户、订单、现金、T+1、手续费、仓位与风控仍在每次 Paper 计划请求中实时计算。临时本地 mock Paper API 实测重复计划请求约从 1.3 秒降至 5 毫秒。
- 2026-08-23 研究等待上限修复：核心板块/个股历史研究增加 12 秒总等待上限；超时返回 `degraded` 空研究并禁止新增 Paper 仓位，辅助新闻/外围研究仍使用 1.5-4 秒有界降级。研究缓存键同时包含持仓标的，持仓变化后不会沿用旧的个股优先研究。
- 2026-08-23 前端稳定性验证：REST 客户端保留浏览器取消请求，不再把页面切换误报为 `fetch failed`；市场研究懒加载使用单次 chunk 恢复和局部错误边界。临时本地 mock Paper 登录、`/learning` 与 `/market` 冒烟检查无页面错误；服务端 60 个文件/855 项、前端 31 个文件/111 项测试通过，TypeScript 与 Vite 生产构建、`git diff --check` 通过。
- 2026-08-29 周度 Paper 资金效率升级已完成：验证路由不再固定使用最低费用整手，而是按候选评分与策略评分在有界预算档位内分层；`kairosQualifiedProbe` 的额度高于仅用于样本收集的 `kairosValidationBasket`。排序、可负担候选统计和最终计划共用同一数量计算，仍保留单票上限、现金缓冲、费用纪律、T+1、阶段仓位、熔断、幂等和每日 10 笔硬上限。该改动只改善 Paper 研究样本的资金效率，不承诺成交、胜率或收益。
- 本轮本地验证：Server Vitest 61 个文件、863 项通过；Web Vitest 31 个文件、112 项通过；`tsc -b`、Vite 生产构建（2,322 个模块）和 `git diff --check` 均通过。下一交易时段仍需观察实际计划金额、成交情况和按策略归因结果。
- 2026-08-29 Paper 计划策略轮换修复：候选先完成最终排序和截断，再复制历史策略使用次数；只有真正生成 `paper-buy-plan` 的候选才递增本轮使用，已有持仓或被费用、历史、现金及档位规则拦截的候选不会抢占首选策略。最强可执行候选不会再因为未入选、较弱或不可执行候选提前占用次数而失去首选策略，分数接近的后续可执行候选仍会优先使用本轮较少使用的合格策略。新增两候选及已有持仓集成回归测试；不回写持久化使用统计，也不放宽策略资格、现金、手续费、T+1、仓位或 Paper 执行边界。
- 2026-08-29 资金口径展示修复：研究管线的周度 Paper 复盘和今日 Paper 计划均直接展示实际 `initialCapital` 与当前权益，避免把上一轮 100,000 元优化目标误解为当前账户余额；当前本地 `.env.local` 仍为 10,000 元，已有 JSON Paper 账户不会因代码展示改动而被重置。
- 2026-08-29 生产单笔上限核对：`deploy/production.env.example` 仍配置 `TRADING_STARTING_CASH=10000` 与 `MAX_ORDER_NOTIONAL=2500`。若线上持久化 Paper 账户实际为 100,000 元，单笔仍会独立受 2,500 元上限约束；该参数属于生产风险预算，未在本轮自动修改或部署。
- 2026-08-29 周度成功率解释补强：策略成交分布新增每个策略的 FIFO 已闭合胜率；没有闭合买卖样本时明确显示“样本不足”，不使用未实现收益、回测结果或未平仓订单替代成功率。周度接口、OpenAPI、共享查询、请求取消和研究页面字段均有回归覆盖。
- 2026-08-29 上周 Paper 复盘周期修复：`/api/research/weekly-paper-review?period=previous` 现在按请求日期返回上一完整的周一至周五交易周，默认无参数仍保留本周至今兼容行为；研究管线页默认请求上周，并显示有效单笔风控上限、占初始资金比例和资金约束类型。摘要会随周期显示“本周/上周”，不会把上一周数据标错。
- 2026-08-29 订单金额诊断补强：周度复盘通过 `system.risk.getEffectiveMaxOrderNotional()` 展示真实生效单笔上限。生产模板当前仍为 `MAX_ORDER_NOTIONAL=2500`，若线上账户为 100,000 元，单笔理论上限只占 2.5%；本轮没有静默提高该风险预算。生产模板交易历史留存改为有界 30 天，不能恢复旧 7 天配置已清理的记录。
- 2026-08-29 本轮复盘优化验证：服务端 Vitest 61 个文件/866 项、前端 Vitest 31 个文件/114 项通过；TypeScript、Vite 生产构建（2,322 个模块）和 `git diff --check` 通过。增加了上一完整周周期、有效单笔上限、策略资金放大不越界和周度页面证据的回归测试。
- 2026-08-29 周度留存证据边界补强：`/api/research/weekly-paper-review` 现在接收实际 `TRADING_HISTORY_RETENTION_DAYS` 并返回 `within-retention`、`may-be-pruned` 或 `unknown`。当上一完整周已超出留存窗口时，研究页明确显示“可能已被清理”，不会把空记录误判为零交易或策略失败；该诊断只读，不改变 Paper 计划、订单、风控或每日上限。本轮服务端 Vitest 61 个文件/867 项、前端 Vitest 31 个文件/114 项、TypeScript、Vite 生产构建（2,322 个模块）和 `git diff --check` 均通过，代码尚未部署到服务器。
- 2026-08-29 资金约束归因补强：周度复盘的 `sizingConstraint` 进一步区分配置单笔上限、现金/现金缓冲、阶段订单额度、一手/金额不足、策略/信号门槛和暂无成交样本；配置上限优先级只用于解释 10 万元账户为何可能出现几千元订单，不会放宽现金、费用、仓位、T+1、熔断或每日 10 笔限制。完整服务端 Vitest 61 个文件/868 项、前端 31 个文件/114 项通过，`tsc -b`、Vite 生产构建（2,322 个模块）和 `git diff --check` 通过；代码仍未部署到服务器。
- 2026-08-29 计划与成交落地率补强：`paper-auto-execution.run` 审计新增每日计划快照，周度复盘按每天最大快照汇总计划买入/卖出额、日峰值计划金额、自动 Paper 买入成交额和计划落地率，避免重复轮询把计划金额重复放大；自动成交只识别 `kairos-auto-paper:` 订单，手工订单不混入执行落地率。研究管线页兼容旧后端缺字段，并显示计划天数、计划金额、自动成交金额和落地率；诊断会区分有运行无快照、有计划无成交和计划明显高于成交。
- 2026-08-29 跳过操作留痕补强：自动 Paper 执行审计的 `skippedReasons` 对来自计划的操作保留策略、策略键、数量、价格和估算金额，且纳入审计签名；下一轮可继续区分计划生成后被现金、阶段额度、单轮额度、行情、重复提交或仓位节奏拦截的金额。该改动不改变执行判断、现金/费用/T+1/仓位/熔断或每日 10 笔硬上限。
- 2026-08-29 计划落地率本地验证：服务端 Vitest 61 个文件、872 项通过；前端 Vitest 31 个文件、114 项通过；`tsc -b`、Vite 生产构建（2,322 个模块）和 `git diff --check` 均通过。新增同日最大计划快照、自动成交过滤、计划无成交、审计签名变化和接口字段回归；本轮代码仍未部署到服务器。
- 2026-08-29 每日金额对账补强：研究管线在周度复盘下按交易日展示计划买入、自动成交、计划落地率以及提交/成交/拒绝笔数；旧后端缺少日级新字段时回退为 0/暂无，不阻断整页渲染。该视图只用于 Paper 执行诊断，不把计划金额当作成交或收益。
- 2026-08-29 阻塞金额归因补强：周度阻塞原因新增记录估算金额，现金拒单读取请求数量与价格，自动计划跳过读取审计中的 `estimatedNotional`；它用于发现哪类约束累计影响更大，不代表实际损失或可直接释放的资金，也不改变任何风控判断。
- 2026-08-30 Paper 研究韧性与策略轮换收口：无可交易快照时，策略排行榜和 Paper 计划返回可解释的 `watch-only` 降级结果，不再因空行情直接报 HTTP 500；相同标的集合的顺序变化不会造成研究缓存失效；登录成功后的重复交易 bootstrap 请求已移除；懒加载恢复在浏览器禁止 `sessionStorage` 时使用安全的内存降级。服务端 Vitest 62 个文件/874 项、前端 31 个文件/116 项、TypeScript、Vite 生产构建（2,322 个模块）和 `git diff --check` 均通过。当前代码仅完成本地验证，尚未部署到服务器；生产参数、Paper 风控和真实交易关闭状态未改动。
- 2026-08-30 bootstrap 请求取消修复：`useTradingBackend` 现在把 TanStack Query 的 `AbortSignal` 传递到健康检查、能力声明、行情、账户、持仓、订单和风控限额的全部 bootstrap 请求；离开登录后页面、登出或重挂载时不会遗留旧请求，避免偶发的陈旧响应和额外 API 负载。新增全量信号透传回归测试，未改变查询数据、Paper 执行或任何交易约束。
- 2026-09-06 本周 Paper 活跃度审计：本地 `.env.local` 已切换为 `validation-probe`、目标 6 笔、每日硬上限 10 笔，仍保持单轮 1 笔、现金缓冲、阶段额度、T+1、手续费、仓位和熔断检查。此前本地状态只留下周末启动审计，没有本周盘中自动执行证据，因此不能把零成交直接归因于策略无信号。公网健康接口仍为 `paper + akshare` 且真实交易关闭，本轮尚未部署；生产凭据和生产持久化账户未改动。
- 2026-09-07 Paper 活跃度与冷启动修复：自动执行器在没有正价格可交易行情时直接记录等待行情并跳过昂贵研究；每个计划操作提交前再次校验行情价格；持仓估值遇到 0、NaN 或无限价格时回退到持仓成本；持久化限价单遇到无效价格不会被错误撮合。研究缓存对 `degraded` 结果使用 5 秒 TTL，健康结果保持 30 秒，桥接恢复后可更快重评。策略轮换与有界验证仓位测试保持通过，不移除 Paper 每日 10 笔、单轮 1 笔或其他风控边界。
- 2026-09-07 本轮验证：服务端全量 Vitest 62 个文件/882 项通过，前端 31 个文件/116 项通过；`tsc -b`、Vite 生产构建（2,322 个模块）和 `git diff --check` 通过。API `8787`、AkShare 桥 `8800`、Vite `4173` 均在本地监听；桥接健康检查报告 5,555 个股票缓存、562 个指数缓存、无最近股票/指数错误。本轮代码尚未部署服务器，生产凭据与生产 Paper 状态未改动。
- 2026-09-20 行情恢复链路修复：底层桥接请求此前会把 HTTP 200 的部分历史结果缓存 5 至 10 分钟，外层降级缓存即使过期仍拿到旧结果。现对非空 `warning` 最多缓存 5 秒，健康 TTL 从完成时计算，执行中请求保持合并并避免被容量淘汰；64 个槽全部执行中时新 key 不进入缓存。新增部分历史恢复、慢请求合并、容量保护、旧请求晚完成隔离与市场形态恢复回归，不改变交易轮询周期或放宽历史证据要求。
- 2026-09-20 研究管线升级：周度复盘独立加载，支持本周/上周切换和单独刷新；切换时不显示另一周期的占位数据，同周期刷新失败保留结果并提示缓存时间。移除固定“24/24、7/24、第 8 天”演示进度，改为接口候选、初筛门槛与成交/闭合计数；缺数据不再显示默认 37 分，合成样本胜率明确标注来源。研究指标改为响应式列，修复手机上 100,000 元金额溢出。
- 2026-09-20 验证与运行边界：后端 62 个文件/890 项、前端 32 个文件/120 项、TypeScript、Vite 构建（2,322 个模块）通过；桌面和 390px 手机浏览器使用隔离模拟夹具验证切换和布局，无横向溢出及控制台错误。任务开始时本地 API/桥接/前端均未运行，本地持久化证据仅到 9 月 6 日，不能评价最近两周线上成交。现在 `127.0.0.1:4173` 与 API `8787` 仅运行本地预览：mock 行情、内存账户、自动交易/微信/外盘采样关闭，沿用登录保护，不改 `.env.local`、生产凭据或持久化账户；AkShare 桥未启动。本地验证阶段未提交或部署，线上状态随后由发布工作流单独核验。
- 2026-09-20 发布核验：第一次生产工作流 `35520517204` 因前端测试依赖旧的 `VITE_API_BASE_URL` 在验证阶段失败，没有部署；修复后的提交 `06a1d3992e7fb1a3e2817df0ba933a047c108954` 由工作流 `35520743365` 在 `codex/real-market-regime` 上完成测试、构建和腾讯云部署，两个 job 均成功。2026-09-21 直接检查 `https://kairosq.cn/healthz` 与 `/api/health` 均返回 200，API 报告 `paper + akshare`、认证开启、真实交易关闭。仓库随后统一为单一 `main` 分支，生产工作流触发器同步改为 `main`。
