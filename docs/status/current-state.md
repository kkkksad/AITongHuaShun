...
- **腾讯云单机生产部署** —— `111.229.76.161` 已运行 Nginx、Fastify 和 AkShare 三个健康容器；公网仅使用 80/443，API 与行情桥只在容器网络内通信。生产模式固定为 `paper + akshare`，真实交易关闭，认证保护开启，Paper JSON 历史默认保留 7 天。IP 入口暂用自签名 HTTPS，仍需域名和受信任证书。
