...
Fastify 的 `bridgeRequest` 是 Python 桥接缓存之外的第二层短时只读缓存。它按 URL、凭据作用域和请求实现区分键，最多保留 64 个 Promise；相同在途读取只访问一次桥接，失败立即删除，过期与超限按有界顺序淘汰。板块快照、历史日线、新闻、全球市场和外部市场使用各自的短 TTL；账户、持仓、订单、审计以及任何写请求不得进入该缓存。
